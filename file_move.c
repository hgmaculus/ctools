/* file_move by H. Gabriel Máculus <gabrielmaculus@gmail.com>
 * 
 */
#include <stdio.h>
...
/*
  file_move do the entire file move
  with one read and one write
  all file must be fill in memory assigned with malloc
 */
int file_move(const char *sname, const char *dname){
...
{
  if(argc != 3) {
    puts("Usage: file_move source destination");
    return -3; 
  }
